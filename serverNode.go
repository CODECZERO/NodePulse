...
func savePassiveLog(activity string, systemUsage map[string]interface{}) {
	...
	if _, err := os.Stat(fileName); os.IsNotExist(err) {
		...
		// Write header row
		headers := "Timestamp,Activity,MemoryTotal,MemoryUsed,MemoryUsedPercent,CPUUsage,LoadAvg,Uptime,PowerUsage\n"
		if _, err := file.WriteString(headers); err != nil {
	...
	// Format: Timestamp, Activity, System Usage
	logEntry := fmt.Sprintf(
		"%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%.2f\n",
